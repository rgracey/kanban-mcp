<script lang="ts">
	...
	async function remove() {
		if (
			!confirm(
				`Delete board "${board.name}"? This will delete all its epics and tickets and cannot be undone.`
			)
		)
			return;
		deleting = true;
	...
	>
		...
		<div class="mt-6 flex items-center justify-between">
			<button
				...
				onclick={remove}
				disabled={deleting || submitting}>{deleting ? 'Deleting...' : 'Delete board'}</button
			>
